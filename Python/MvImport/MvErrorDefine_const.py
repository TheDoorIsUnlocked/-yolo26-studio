...
MV_E_NOOUTBUF = 0x8000000D  ## @~chinese 没有可输出的缓存         @~english No Available Buffer
...
MV_E_IMAGE_DECODEC = (
    0x80000015  ## @~chinese 解码错误(SDK校验图像异常)@~english Decoding error (SDK verification image exception)
)
...
MV_E_IMAGE_HEIGHT = (
    0x80000017  ## @~chinese 图像高度异常（残帧丢弃） @~english image height anomaly (discard incomplete images)
)
...
MV_E_SUPPORT_MODIFY_DEVICE_IP = (
    0x8000020E  # < 在固定IP模式下不支持修改设备IP模式  @~english Current Mode Not Support Modify Ip
)
