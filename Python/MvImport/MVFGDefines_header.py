...
_MV_CXP_INTERFACE_INFO_._fields_ = [
    ...
    (
        "chSerialNumber",
        c_ubyte * MV_FG_MAX_IF_INFO_SIZE,
    ),  # < \~chinese 序列号                   \~english Serial number
    ("nPCIEInfo", c_uint),  # < \~chinese 采集卡的PCIE插槽信息       \~english PCIe slot information of interface
    (
        "chModelName",
        c_ubyte * MV_FG_MAX_IF_INFO_SIZE,
    ),  # <  \~chinese Interface型号           \~english Interface model name
    (
        "chManufacturer",
        c_ubyte * MV_FG_MAX_IF_INFO_SIZE,
    ),  # < \~chinese Interface厂商            \~english Interface manufacturer name
    (
        "chDeviceVersion",
        c_ubyte * MV_FG_MAX_IF_INFO_SIZE,
    ),  # < \~chinese Interface版本            \~english Interface device version
    (
        "chUserDefinedName",
        c_ubyte * MV_FG_MAX_IF_INFO_SIZE,
    ),  # < \~chinese Interface自定义名称       \~english Interface user defined name
]
...
_MV_CXP_DEVICE_INFO_._fields_ = [
    ...
    (
        "chManufacturerInfo",
        c_ubyte * MV_FG_MAX_DEV_INFO_SIZE,
...
_MV_GEV_DEVICE_INFO_._fields_ = [
    ...
    (
        "nIpCfgCurrent",
        c_uint,
    ),  # < \~chinese 当前IP配置，参考支持的IP配置说明   \~english For the current IP configuration, refer to the supported IP configuration description
...
_MV_FG_SAVE_BITMAP_INFO_._fields_ = [
    ...
    ),  # < [IN]  \~chinese 输入图片信息     \~english Input image information
...
_MV_FG_CCM_INFO_._fields_ = [
    ...
    (
        "nCCMScale",
        c_uint,
    ),  # < [IN]  \~chinese 量化系数（2的整数幂,最大65536）    \~english Quantitative scale(Integer power of 2, <= 65536)
    ("nReserved", c_uint * 4),  # < \~chinese 保留字段     \~english Reserved
]
...
_MV_FG_CONVERT_PIXEL_INFO_._fields_ = [
    ...
    ),  # < [IN][OUT]  \~chinese 输出图片信息    \~english Output image information
    ("enCfaMethod", MV_FG_CFA_METHOD),  # < [IN]  \~chinese 插值方法   \~english CFA method
    ...
    (
        "stCCMInfo",
        MV_FG_CCM_INFO,
    ),  # < [IN]  \~chinese CCM信息,仅windows支持    \~english CCM information，support only windows
    ("nReserved", c_uint * 4),  # < \~chinese 保留字段     \~english Reserved
]
...
_MV_FG_RECONSTRUCT_INFO_._fields_ = [
    ...
    (
        "stOutputImageInfo",
        MV_FG_OUTPUT_IMAGE_INFO * MV_FG_MAX_SPLIT_NUM,
    ),  # < [IN][OUT]  \~chinese 输出图片信息    \~english Output image information
...
_MV_FG_ENUMVALUE_._fields_ = [
    ...
    (
        "strCurSymbolic",
        c_byte * MV_FG_MAX_SYMBOLIC_STRLEN,
    ),  # < [OUT] \~chinese 当前值的符号名称   \~english The symbolic name of the current value
    (
        "nSupportedNum",
        c_uint,
    ),  # < [OUT] \~chinese 支持的枚举类型个数      \~english Number of supported enumeration types
    (
        "nSupportValue",
        c_uint * MV_FG_MAX_SYMBOLIC_NUM,
    ),  # < [OUT] \~chinese 支持的枚举类型的值      \~english Values of supported enumeration types
    (
        "strSymbolic",
        (c_uint * MV_FG_MAX_SYMBOLIC_NUM) * MV_FG_MAX_SYMBOLIC_STRLEN,
    ),  # < [OUT] \~chinese 支持的枚举类型的符号名称    \~english Symbolic names of supported enumeration types
    ("nReserved", c_uint * 4),  # < \~chinese 保留字段     \~english Reserved
]
...
_MV_FG_EVENT_INFO_._fields_ = [
    ...
    (
        "pEventData",
        c_void_p,
    ),  # < [OUT] \~chinese 事件数据，内部缓存，需要及时进行数据处理   \~english Event data, internal buffer, need to be processed in time
    ("nEventDataSize", c_uint),  # < [OUT] \~chinese 事件数据长度     \~english Event data size
    ("nReserved", c_uint * 4),  # < \~chinese 保留字段     \~english Reserved
]
...
_MVFG_CIRCLE_INFO._fields_ = [
    ...
    (
        "fR1",
        c_float,
    ),  # <  \~chinese 宽向半径，根据图像的相对位置[0, 1.0]，半径与圆心的位置有关，需保证画出的圆在显示框范围之内，否则报错  \~english Windth Radius, Range[0, 1.0]
    (
        "fR2",
        c_float,
    ),  # <  \~chinese 高向半径，根据图像的相对位置[0, 1.0]，半径与圆心的位置有关，需保证画出的圆在显示框范围之内，否则报错  \~english Height Radius, Range[0, 1.0]
    ("stColor", MVFG_COLORF),  # <  \~chinese 辅助线颜色                      \~english Color of Auxiliary Line
    (
        "nLineWidth",
        c_uint,
...
_MVFG_LINES_INFO._fields_ = [
    ...
    (
        "stEndPoint",
        MVFG_POINTF,
    ),  # <  \~chinese 线条辅助线的终点坐标            \~english The End Point of Auxiliary Line
    ("stColor", MVFG_COLORF),  # <  \~chinese 辅助线颜色                      \~english Color of Auxiliary Line
    (
        "nLineWidth",
        c_uint,
