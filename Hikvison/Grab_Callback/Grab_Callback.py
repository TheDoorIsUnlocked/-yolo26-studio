...
if __name__ == "__main__":
    ...
    for i in range(1):
        ...
        if MV_FG_SUCCESS != ret:
            print(f"Warning: MV_FG_UpdateInterfaceList error, ret[0x{ret:x}]")
            break
        ...
        if MV_FG_SUCCESS != ret or 0 == nInterfaceNum.value:
            print(f"Warning: MV_FG_GetNumInterfaces error, ret[0x{ret:x}]")
            break
        ...
        if MV_FG_SUCCESS != ret:
            print(f"Warning: MV_FG_OpenInterface error, ret[0x{ret:x}]")
            break

        # ch: 枚举采集卡上的相机 | en:Enum camera of interface
        ret = Iface.UpdateDeviceList(bChanged)
        if MV_FG_SUCCESS != ret:
            print(f"Warning: MV_FG_UpdateDeviceList error, ret[0x{ret:x}]")
            break
        ...
        if MV_FG_SUCCESS != ret:
            print(f"Warning: MV_FG_GetNumDevices error, ret[0x{ret:x}]")
            break
        ...
        if MV_FG_SUCCESS != ret:
            print(f"Warning: MV_FG_OpenDevice error, ret[0x{ret:x}]")
            break
    ...
    if Strm._STREAMHANDLE.value is not NULL:
        ...
        if MV_FG_SUCCESS != ret:
            print(f"Warning: MV_FG_CloseStream error, ret[0x{ret:x}]")
    ...
    if Dev._DEVHANDLE.value is not NULL:
        ret = Dev.CloseDevice()
        if MV_FG_SUCCESS != ret:
            print(f"Warning: MV_FG_CloseDevice error, ret[0x{ret:x}]")
        Dev.DEVHANDLE = None
    ...
    if Iface._IFHANDLE.value is not NULL:
        ...
        if MV_FG_SUCCESS != ret:
            print(f"Warning: MV_FG_CloseInterface error, ret[0x{ret:x}]")
        Iface.IFHANDLE = None
