...
if __name__ == "__main__":
    try:
        ...
        print("start multicast sample in (c)control or in (m)onitor mode? (c/m)")
        ...
        dest_ip = (
            (int(device_ip_list[0]) << 24)
            | (int(device_ip_list[1]) << 16)
            | (int(device_ip_list[2]) << 8)
            | int(device_ip_list[3])
        )
        print(f"dest ip: {strIp}")
        ...
        if MV_OK != ret:
            raise Exception(f"set transmission type fail! ret [0x{ret:x}]")
