...
if __name__ == "__main__":
    try:
        ...
        if ret != 0:
            raise Exception(f"set Bayer convert quality fail! ret[0x{ret:x}]")
        ...
        if None != stOutFrame.pBufAddr and 0 == ret:
            ...
            if ret != 0:
                raise Exception(f"convert pixel fail! ret[0x{ret:x}]")
            ...
            try:
                ...
                raise Exception(f"save file executed failed:{e.message}")
            finally:
                file_open.close()
        else:
            print(f"get one frame fail, ret[0x{ret:x}]")
