...
if __name__ == "__main__":
    try:
        ...
        if ret != 0:
            raise Exception(f"set trigger mode on fail! ret[0x{ret:x}]")

        # ch:设置触发源为Line0 | en:Set trigger source as Line0
        ret = cam.MV_CC_SetEnumValueByString("TriggerSource", "Line0")
        if ret != 0:
            raise Exception(f"set trigger source fail! ret[0x{ret:x}]")
