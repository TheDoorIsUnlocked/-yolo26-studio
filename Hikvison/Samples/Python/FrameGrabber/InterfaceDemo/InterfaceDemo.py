...
if __name__ == "__main__":
    try:
        ...
        if ret != 0:
            raise Exception(f"get StreamSelector fail! ret[0x{ret:x}]")

        # ch:设置属性 | en:Set Feature
        ret = cam.MV_CC_SetEnumValue("StreamSelector", stEnumValue.nCurValue)
        if ret != 0:
            raise Exception(f"set StreamSelector fail! ret[0x{ret:x}]")
        else:
        ...
        if ret != 0:
            raise Exception(f"close interface fail! ret[0x{ret:x}]")
        else:
