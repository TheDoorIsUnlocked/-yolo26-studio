...
def event_callback(pEventInfo, pUser):
    ...
    if stPEventInfo:
        print(
            "EventName[%s], EventId[%u], BlockId[%d], Timestamp[%d]"
            % (stPEventInfo.EventName.decode("UTF-8"), stPEventInfo.nEventID, nBlockId, nTimestamp)
        )
...
if __name__ == "__main__":
    try:
        ...
        if ret != 0:
            print(f"enum interfaces fail! ret[0x{ret:x}]")
            sys.exit()
        ...
        if ret != 0:
            raise Exception(f"create interface handle fail! ret[0x{ret:x}]")

        # ch:打开采集卡 | en:Open interface
        ret = interface_instance.MV_CC_OpenInterface()
        if ret != 0:
            raise Exception(f"open interface fail! ret[0x{ret:x}]")
        else:
        ...
        if ret != 0:
            raise Exception(f"Set interface event notification on fail! ret[0x{ret:x}]")
        ...
        if ret != 0:
            raise Exception(f"register event callback fail! ret [0x{ret:x}]")
