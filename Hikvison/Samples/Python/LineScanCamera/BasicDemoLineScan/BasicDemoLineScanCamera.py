    ...
    try:
        ...
        for i in range(deviceList.nDeviceNum):
            ...
            if mvcc_dev_info.nTLayerType == MV_GIGE_DEVICE or mvcc_dev_info.nTLayerType == MV_GENTL_GIGE_DEVICE:
                ...
                print(f"device user define name: {strUserDefinedName}")
        ...
        if ret != 0:
            raise Exception(f"error: create handle fail! ret[0x{ret:x}]")

        # ch:打开设备 | en:Open device
        ret = cam.MV_CC_OpenDevice(MV_ACCESS_Exclusive, 0)
        if ret != 0:
            raise Exception(f"error: open device fail! ret[0x{ret:x}]")
        ...
        if ret != 0:
            raise Exception(f"error: set trigger mode fail! ret[0x{ret:x}]")

        # ch 设置数字增益 | en: Set digital shift
        ret = cam.MV_CC_SetBoolValue("DigitalShiftEnable", True)
        if ret != 0:
            raise Exception(f"error: set digital shift enable fail! ret[0x{ret:x}]")

        ret = cam.MV_CC_SetFloatValue("DigitalShift", 0)
        if ret != 0:
            raise Exception(f"error: set digital shift fail! ret[0x{ret:x}]")

        # ch 设置行频 | en: Set  acquisition line rate
        ret = cam.MV_CC_SetIntValue("AcquisitionLineRate", 10000)
        if ret != 0:
            raise Exception(f"error: set acquisition line rate fail! ret[0x{ret:x}]")

        ret = cam.MV_CC_SetBoolValue("AcquisitionLineRateEnable", True)
        if ret != 0:
            raise Exception(f"error: set acquisition line rate enable fail! ret[0x{ret:x}]")

        # ch 设置HB模式 | en: Set image compression mode:HB
        ret = cam.MV_CC_SetEnumValueByString("ImageCompressionMode", "HB")
        if ret != 0:
            raise Exception(f"error: set  image compression mode: HB fail! ret[0x{ret:x}]")
        ...
        if ret != 0:
            raise Exception(f"get payload size fail! ret[0x{ret:x}]")
        nPayloadSize = stParam.nCurValue

        # ch:开始取流 | en:Start grab image
        ret = cam.MV_CC_StartGrabbing()
        if ret != 0:
            raise Exception(f"error: start grabbing fail! ret[0x{ret:x}]")
        ...
        if None != stOutFrame.pBufAddr and 0 == ret:
            ...
            if ret != 0:
                raise Exception(f"error: high bandwidth decode fail! ret[0x{ret:x}]")
            else:
            ...
            if ret != 0:
                raise Exception(f"error: save image to file fail! ret[0x{ret:x}]")
            else:
                print("save image to file is OK")

            cam.MV_CC_FreeImageBuffer(stOutFrame)
        else:
            print(f"error: get one frame fail, ret[0x{ret:x}]")
        ...
        if ret != 0:
            raise Exception(f"error: stop grabbing fail! ret[0x{ret:x}]")

        # ch:关闭设备 | Close device
        ret = cam.MV_CC_CloseDevice()
        if ret != 0:
            raise Exception(f"error: close device fail! ret[0x{ret:x}]")
