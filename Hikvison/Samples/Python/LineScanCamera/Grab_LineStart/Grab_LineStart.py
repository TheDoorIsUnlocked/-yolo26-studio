    ...
    while True:
        ...
        if out_frame_info.pBufAddr is not None and 0 == ret:
            print(
                ...
                % (
                    out_frame_info.stFrameInfo.nWidth,
                    out_frame_info.stFrameInfo.nHeight,
                    out_frame_info.stFrameInfo.nFrameNum,
                )
            )
            ret = cam.MV_CC_FreeImageBuffer(out_frame_info)
        else:
            print(f"error: no data[0x{ret:x}]")
        if g_bExit is True:
            break
...
if __name__ == "__main__":
    try:
        ...
        if ret != 0:
            raise Exception(f"error: get AcquisitionLineRateEnable fail! ret[0x{ret:x}]")

        # ch:设置行触发 | en:Set trigger selector is line start
        ret = cam.MV_CC_SetEnumValueByString("TriggerSelector", "LineStart")
        if ret != 0:
            raise Exception(f"error: set trigger selector fail! ret[0x{ret:x}]")
        ...
        if ret != 0:
            raise Exception(f"error: set trigger source fail! ret[0x{ret:x}]")
