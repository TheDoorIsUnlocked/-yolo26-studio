    ...
    try:
        ...
        if ret == 0:
            ...
            if stOutFrame.stFrameInfo.enPixelType in HB_format_list:
                ...
                if 0 != ret:
                    raise Exception(f"Get PayloadSize fail! ret[0x{ret:x}]")
                ...
                if ret != 0:
                    ...
                    raise Exception(f"HB Decode fail! ret[0x{ret:x}]")
                else:
            ...
            if ret != 0:
                cam.MV_CC_FreeImageBuffer(stOutFrame)
                raise Exception(f"MV_CC_ReconstructImage fail! ret[0x{ret:x}]")
            else:
                print("Reconstruct image success")

            # ch: 保持图像到文件 | en: Save image to file
            file_name = "Image_w%d_h%d_fn%d.bmp" % (
                stReconstructParam.nWidth,
                stReconstructParam.nHeight,
            ...
            for i in range(exposure_num):
                ctypes.memmove(
                    ctypes.addressof(img_buff) + i * sub_len, stReconstructParam.stDstBufList[i].pBuf, sub_len
                )
            ...
            if ret != 0:
                print(f"Save stitched image fail! ret[0x{ret:x}]")
            else:
