...
MV_FG_ERR_IMG_FORMAT = (
    0x80190207  # < \~chinese 图像格式不正确或者不支持       \~english Incorrect format or unsupported
)
MV_FG_ERR_IMG_SIZE = (
    0x80190208  # < \~chinese 图像宽高不正确或者超出范围     \~english Incorrect width and height or out of range
)
MV_FG_ERR_IMG_STEP = (
    0x80190209  # < \~chinese 图像宽高与step参数不匹配       \~english Width and height do not match step parameters
)
MV_FG_ERR_IMG_DATA_NULL = (
    0x80190210  # < \~chinese 图像数据存储地址为空(某个分量) \~english Data storage address is empty (a component)
)
MV_FG_ERR_IMG_ABILITY_ARG = (
    0x80190211  # < \~chinese 图像算法ABILITY存在无效参数    \~english Invalid parameter for algorithm ABILITY
)
