...
def GrabbingThread(Strm=0):
    ...
    if MV_FG_SUCCESS != ret:
        print(f"Warning: MV_FG_StartAcquisition error, ret[0x{ret:x}]")
        return ret
    while True:
        ret = Strm.GetFrameBuffer(stFrameInfo, TIMEOUT)
        if MV_FG_SUCCESS != ret:
            print(f"Warning: MV_FG_GetFrameBuffer error, ret[0x{ret:x}]")
            continue
        else:
            print(
                "FrameNumber:[%d],    Width[%d],    Height[%d]"
                % (stFrameInfo.nFrameID, stFrameInfo.nWidth, stFrameInfo.nHeight)
            )
        ret = Strm.ReleaseFrameBuffer(stFrameInfo)
        if MV_FG_SUCCESS != ret:
            print(f"Warning: MV_FG_ReleaseFrameBuffer error, ret[0x{ret:x}]")
        if True == g_bExit:
            break
    ret = Strm.StopAcquisition()
    if MV_FG_SUCCESS != ret:
        print(f"Warning: MV_FG_StopAcquisition error, ret[0x{ret:x}]")
        return ret
...
def PrintInterfaceInfo(nInterfaceNum=0):
    for i in range(nInterfaceNum):
        ...
        if MV_FG_SUCCESS != ret:
            print(f"Warning: MV_FG_GetInterfaceInfo error, ret[0x{ret:x}]")
            return False
        ...
        if MV_FG_CXP_INTERFACE == stInterfaceInfo.nTLayerType:
            ...
            print(
                "[CXP]No.%d Interface: \n\tDisplayName: %s\n\tInterfaceID: %s\n\tSerialNumber:%s\n"
                % (i, chDisplayName, chInterfaceID, chSerialNumber)
            )
...
def PrintDeviceInfo(Iface=0, nDeviceNum=0):
    for i in range(nDeviceNum):
        ...
        if MV_FG_CXP_DEVICE == stDeviceInfo.nDevType:
            ...
            print(
                "[CXP]No.%d Device: \n\tUserDefinedName: %s\n\tModelName: %s\n\tSerialNumber: %s\n"
                % (i, chUserDefinedName, chModelName, chSerialNumber)
            )
...
if __name__ == "__main__":
    ...
    for i in range(1):
        ...
        if MV_FG_SUCCESS != ret:
            print(f"Warning: UpdateInterfaceList error, ret[0x{ret:x}]")
            break
        ...
        if MV_FG_SUCCESS != ret or 0 == nInterfaceNum.value:
            print(f"Warning: GetNumInterfaces error, ret[0x{ret:x}]")
            break
        ...
        if MV_FG_SUCCESS != ret:
            print(f"Warning: OpenInterface error, ret[0x{ret:x}]")
            break

        # ch: 枚举采集卡上的相机 | en:Enum camera of interface
        ret = Iface.UpdateDeviceList(bChanged)
        if MV_FG_SUCCESS != ret:
            print(f"Warning: UpdateDeviceList error, ret[0x{ret:x}]")
            break
        ...
        if MV_FG_SUCCESS != ret:
            print(f"Warning: GetNumDevices error, ret[0x{ret:x}]")
            break
        ...
        if MV_FG_SUCCESS != ret:
            print(f"Warning: OpenDevice error, ret[0x{ret:x}]")
            break
        ...
        if MV_FG_SUCCESS != ret:
            print(f"Warning: Set TriggerMode Off error, ret[0x{ret:x}]")
            break
        ...
        if MV_FG_SUCCESS != ret or 0 == nStreamNum.value:
            print(f"Warning: GetNumStreams error, ret[0x{ret:x}]")
            break

        # ch:打开流通道(目前只支持单个通道) | en:Open stream(Only a single stream is supported now)
        ret = Strm.OpenStream(Dev, 0)
        if MV_FG_SUCCESS != ret:
            print(f"Warning: OpenStream error, ret[0x{ret:x}]")
            break

        # ch:设置SDK内部缓存数量 | en:Set internal buffer number
        ret = Strm.SetBufferNum(BUFFER_NUMBER)
        if MV_FG_SUCCESS != ret:
            print(f"Warning: SetBufferNum error, ret[0x{ret:x}]")
            break

        # ch:创建取流线程 | en:Create acquisition thread
        try:
        ...
        # ch:关闭取流线程 | en:Close acquisition thread
    ...
    if Strm._STREAMHANDLE.value is not NULL:
        ...
        if MV_FG_SUCCESS != ret:
            print(f"Warning: CloseStream error, ret[0x{ret:x}]")
    ...
    if Dev._DEVHANDLE.value is not NULL:
        ret = Dev.CloseDevice()
        if MV_FG_SUCCESS != ret:
            print(f"Warning: CloseDevice error, ret[0x{ret:x}]")
        Dev.DEVHANDLE = None
    ...
    if Iface._IFHANDLE.value is not NULL:
        ...
        if MV_FG_SUCCESS != ret:
            print(f"Warning: CloseInterface error, ret[0x{ret:x}]")
        Iface.IFHANDLE = None
