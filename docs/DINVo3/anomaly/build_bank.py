...
def main():
    ...
    ap.add_argument("--out", default="models/anomaly_bank.npz", help="特征库输出路径(.npz)")
    ap.add_argument("--model", default="dinov3_vits16", help="骨干名(dinov3_vits16 等 dinov3 骨干,或 dinov2_vits14)")
    ...
    ap.add_argument("--bank-size", type=int, default=16384, help="coreset 后特征库向量数(越大越准越慢)")
    ...
    if not paths:
        ...
        return 2
    print(f"building bank from {len(paths)} images (model={args.model}, bank_size={args.bank_size})")
    model, patch_size, device = da.load_model(
        args.model, args.hub_repo, args.device, weights_path=args.weights, repo_dir=args.repo_dir
    )
    # .fbin(服务端 C++ 格式: int32 M,D + float32 M*D) 由 build_bank 内部一并产出,
    # 这里不再重复写(原实现引用了未定义的 out 变量,必然 NameError)
    info = da.build_bank(model, patch_size, paths, args.out, device, args.bank_size, model_name=args.model)
