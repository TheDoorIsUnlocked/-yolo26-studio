...
class HubPatchWrapper(nn.Module):
    ...
    def forward(self, x):
        return F.normalize(self.inner.forward_features(x)["x_norm_patchtokens"], dim=-1)
...
def export_onnx(
    ...
    if model.startswith("dinov3"):
        ...
        inner, patch_size, device = da.load_backbone_model(model, weights, repo_dir, device)
        ...
        class HfWrapper(nn.Module):
            ...
            def forward(self, x):
                return F.normalize(self.m(x).last_hidden_state[:, 1:, :], dim=-1)
...
def main():
    ...
    ap.add_argument("--model", default="dinov3_vits16", help="dinov3_vits16(本地权重) 或 dinov2_vits14(自动下载)")
    ap.add_argument("--weights", default="", help="dinov3 本地权重 .pth(模型名以 dinov3 开头时必填)")
    ap.add_argument("--repo-dir", default="", help="dinov3 仓库代码目录(默认自动找 anomaly 同级)")
    ap.add_argument("--out", default="models/dinov3_vits16_anomaly.onnx")
    ap.add_argument("--device", default="cuda")
    args = ap.parse_args()

    path, img_size, shape = export_onnx(args.model, args.weights, args.repo_dir, args.out, args.device)
