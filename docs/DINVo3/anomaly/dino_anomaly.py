...
def load_backbone_model(model_name: str = "dinov3_vits16",
                        weights_path: str = "", repo_dir: str = "",
                        device: str = "cuda"):
    """加载 dinov3 骨干**本体**,返回 (model, patch_size, device)。

    与 load_model 的区别:返回模型对象本身(导出 ONNX 需要包 wrapper),
    而非特征提取闭包。两者共用同一套权重加载路径,特征结果一致。"""
    ...
    if not repo_dir or not weights_path:
        raise RuntimeError(
            "dinov3 需要 --repo-dir(dinov3 仓库代码目录)与 "
            "--weights(本地 .pth),未提供")
    ...
    try:
        ...
        raise RuntimeError(
            f"无法从 {repo_dir} 导入 dinov3 仓库代码: {e}")
    builder = getattr(_dino_backbones, model_name, None)
    if builder is None:
        raise RuntimeError(
            f"dinov3 不支持的骨干名 '{model_name}'"
            f"(可用: dinov3_vits16 / dinov3_vits16plus / dinov3_vitb16 ...)")
...
def load_model(model_name: str = "dinov2_vits14",
               ...
               device: str = "cuda", hf_endpoint: str = "https://hf-mirror.com",
               weights_path: str = "", repo_dir: str = ""):
    ...
    - dinov3(权重 .pth 已下载到本地):weights_path=本地权重路径,
      repo_dir=仓库代码目录(默认自动找 anomaly 同级的 dinov3-main)。
    权重已缓存/本地时离线可用。"""
    device = _norm_device(device)

    if model_name.startswith("dinov3"):
        m, patch_size, device = load_backbone_model(
            model_name, weights_path, repo_dir, device)
    ...
    try:
        ...
        print(f"torch.hub 加载失败({e.__class__.__name__}: {e}),"
              f"改用 HuggingFace 镜像 {hf_endpoint}")
...
def _preprocess(img_bgr: np.ndarray, img_size: int, device: str) -> torch.Tensor:
    """BGR uint8 → 归一化 float 张量(1,3,S,S)。整图 resize 到 S×S,
    良品与待检图用同一变换,轻微形变不影响块级比对。"""
...
@torch.no_grad()
def extract_patch_features(model, img_bgr: np.ndarray, patch_size: int,
...
def _greedy_coreset(feats: np.ndarray, budget: int, device: str,
                    seed: int = 0) -> np.ndarray:
    """贪心 k-center coreset:选出的点尽量覆盖整个特征空间
    (PatchCore 同款思想)。候选量大时先随机下采样到 8*budget 再贪心。"""
...
def build_bank(model, patch_size: int, image_paths: list, out_path: str,
               device: str = "cuda", bank_size: int = 16384,
               log=print, model_name: str = "") -> dict:
    device = _norm_device(device)
    ...
    np.savez_compressed(out, feats=bank.astype(np.float16),
                        meta=json.dumps({
    ...
    return {"images": len(all_feats), "patches_raw": int(raw.shape[0]),
            "patches_bank": int(bank.shape[0]), "out": str(out)}
...
@torch.no_grad()
def score_image(model, patch_size: int, bank_t: torch.Tensor,
                img_bgr: np.ndarray, device: str = "cuda",
                top_pct: float = 0.01) -> tuple:
    """单图异常评分。返回 (image_score, patch 距离图 (gh,gw), 网格)。

    image_score = 最异常的 top_pct 比例块的距离均值(比单块 max 稳定)。"""
    feats, (gh, gw) = extract_patch_features(model, img_bgr, patch_size,
                                             device)
...
def _auroc(labels, scores) -> float:
    """AUROC(Mann-Whitney U 秩公式)。优先用 sklearn,缺失时退回自实现,
    避免验证环节因环境缺 sklearn 而整体失败。并列分数取平均秩。"""
    try:
...
def summarize_scores(rows: list) -> list:
    """根据 [(path, label 1=NG, score), ...] 生成指标说明行
    (AUROC/Youden 阈值/零误报阈值/分数分布)。"""
    lines = [f"images: {len(rows)} (ng={int(sum(r[1] for r in rows))}, "
             f"good={int(sum(1 for r in rows if r[1] == 0))})"]
    ...
    if n_ng == 0 or n_good == 0:
        ...
        if n_good:
            ...
            lines.append(f"零误报阈值: {th0:.4f} (良品最高分×1.05), "
                         f"该阈值下漏检={int((scores[labels == 1] < th0).sum())}")
    qs = np.percentile(scores, [0, 25, 50, 75, 100])
    lines.append(f"分数分布 min/25/50/75/max: "
                 f"{qs[0]:.4f}/{qs[1]:.4f}/{qs[2]:.4f}/{qs[3]:.4f}/{qs[4]:.4f}")
    return lines


def validate_dataset(model, patch_size: int, bank_t: torch.Tensor,
                     items: list, out_dir: str, device: str = "cuda",
                     top_k_heatmap: int = 20, log=print) -> list:
    device = _norm_device(device)
    ...
    for i, (p, label) in enumerate(items):
        ...
        log(f"[{i + 1}/{len(items)}] {'NG ' if label else 'OK '} "
            f"score={score:.4f} {p}")
    if not rows:
    ...
    fp_rows = [r for r in rank if r[1] == 0][:max(0, top_k_heatmap
                                                 - len(ng_rows))]
    for p, label, score in ng_rows + fp_rows:
        ...
        cv2.imwrite(str(out / "heatmap" / f"{tag}_{score:.4f}_{stem}.jpg"),
                    vis)
