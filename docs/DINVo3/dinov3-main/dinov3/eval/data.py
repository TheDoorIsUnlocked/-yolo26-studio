...
def get_labels(dataset) -> torch.Tensor:
    """Get the labels of a classification dataset, as a Tensor, using the `get_targets` method if it is present or
    loading the labels one by one with `get_target`, if it exists. If the dataset has a target transform, iterate
    over the whole dataset to get the transformed labels for each element, then stack them as a torch tensor.
    """
...
def create_class_indices_mapping(labels: torch.Tensor) -> dict[int, torch.Tensor]:
    """Efficiently creates a mapping between the labels and tensors containing the indices of all the dataset elements
    that share this label. In the case of multiple labels, it is not guaranteed that there will be exactly the
    specified percentage of labels.
    """
...
def _subset_dataset_per_class(
    ...
    """Helper function to select a percentage of a dataset, equally distributed across classes, or to take the same
    number of elements from each class of the dataset. Returns a boolean mask tensor being True at indices of
    selected elements.
    """
    random_generator = torch.Generator()
    random_generator.manual_seed(seed)
...
def _multilabel_rebalance_subset(
    ...
    labels. Returns a boolean mask tensor being True at indices of selected elements.
    """
...
def split_train_val_datasets(train_dataset, split_percentage: float = 0.1, shuffle_train: bool = True):
    """Splitting a percent of the train dataset to choose hyperparameters, taking the same percentage for each class. If
    `shuffle` is False, taking the first elements of each class as the validaton set.
    """
...
def pad_multilabel_and_collate(batch, pad_value=-1):
    ...
    DatasetWithEnumeratedTargets, with targets that are list of potentially varying sizes. The targets are padded to
    the length of the longest target list in the batch.
    """
