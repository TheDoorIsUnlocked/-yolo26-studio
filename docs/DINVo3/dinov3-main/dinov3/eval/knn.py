...
class KnnModule(torch.nn.Module):
    ...
    Each rank gets a chunk of the train features as well as a chunk of the test features. In `compute_neighbors`, for
    each rank one after the other, its chunk of test features is sent to all devices, partial knns are computed with
    each chunk of train features then collated back on the original device.
    """
...
def _log_and_format_results_dict(input_results_dict, few_shot_n_tries: int) -> dict[str, float]:
    results_dict = {}
    for knn_ in input_results_dict:
